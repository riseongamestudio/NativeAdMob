...
@interface ROInFeedSlotRect : NSObject
...
@implementation ROInFeedSlotRect
@end
...
@interface ROInFeedSlot () <ROInFeedPresentationListener>
@end
...
- (void)configureWithX:(CGFloat)xPt
    ...
    ROInFeedSlotRect *newRect = [[ROInFeedSlotRect alloc] init];
    newRect.x = xPt;
    newRect.y = yPt;
    newRect.width = MAX(1, widthPt);
    newRect.height = MAX(1, heightPt);
    ROInFeedSlotRect *oldRect = _rect;
...
- (void)hide {
    ...
    __weak ROInFeedSlot *weakSelf = self;
    [CATransaction setCompletionBlock:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ROInFeedSlot *strongSelf = weakSelf;
            if (strongSelf == nil
...
- (void)presentCachedAd {
    ...
    if (![presentation show]) {
        ...
        [owner notifySlotPresentationFailed:_index
                                       code:RONativeAdMobInternalPresentationError
