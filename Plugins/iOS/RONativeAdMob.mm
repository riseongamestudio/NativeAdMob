...
const int32_t RONativeAdMobInternalLoadError = -1;
const int32_t RONativeAdMobInternalPresentationError = -2;

@implementation RONativeAdMob {
    ...
    RONativeAdMobListenerCallbacks _callbacks;
...
- (void)clearListenerCallbacks {
    RONativeAdMobListenerCallbacks empty = {0};
    @synchronized (_callbacksLock) {
        _callbacks = empty;
    }
}

- (RONativeAdMobListenerCallbacks)currentCallbacks {
    @synchronized (_callbacksLock) {
        return _callbacks;
    }
}
...
- (void)notifyLoadingStarted {
    RONativeAdMobListenerCallbacks callbacks = [self currentCallbacks];
