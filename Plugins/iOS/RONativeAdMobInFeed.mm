...
- (instancetype)initWithAdUnitId:(NSString *)adUnitId
    ...
    NSMutableArray<ROInFeedSlot *> *slots =
            [NSMutableArray arrayWithCapacity:boundedSlotCount];
    for (NSInteger i = 0; i < boundedSlotCount; ++i) {
        [slots addObject:[[ROInFeedSlot alloc] initWithOwner:self index:i]];
    }
    _slots = slots;
...
- (nullable ROInFeedSlot *)ro_slotAt:(NSInteger)slotIndex
...
- (void)configureSlot:(NSInteger)slotIndex
    ...
    [RONativeAdMob runOnMainThread:^{
        ...
        ROInFeedSlot *slot = [self ro_slotAt:slotIndex
                                   operation:@"Configure"];
        if (slot == nil) return;
...
- (void)showSlot:(NSInteger)slotIndex {
    ...
    [RONativeAdMob runOnMainThread:^{
        ...
        ROInFeedSlot *slot = [self ro_slotAt:slotIndex operation:@"Show"];
...
- (void)hideSlot:(NSInteger)slotIndex {
    ...
    [RONativeAdMob runOnMainThread:^{
        if (self.released) return;
        ROInFeedSlot *slot = [self ro_slotAt:slotIndex operation:@"Hide"];
...
- (void)releaseAd {
    ...
    [RONativeAdMob runOnMainThread:^{
        ...
        for (ROInFeedSlot *slot in self->_slots) [slot releaseOnMain];
...
- (void)nativeAdDidRecordClick:(GADNativeAd *)nativeAd {
    [RONativeAdMob runOnMainThread:^{
        for (ROInFeedSlot *slot in self->_slots) [slot commitAdClick];
    }];
}

- (void)ro_offerCacheToSlots {
    for (ROInFeedSlot *slot in _slots) {
