...
namespace RiseOn.NativeAdMob {
    ...
    /// </summary>
    public sealed class NativeInFeedAdMob : NativeAdMob, INativeInFeedAdMobCallbacks {
        ...
        private INativeInFeedAdMobClient client;
        ...
        void INativeInFeedAdMobCallbacks.OnLoadingStarted()
            => DispatchFromNative(RaiseLoadingStarted);

        void INativeInFeedAdMobCallbacks.OnLoadingCompleted(int errorCode, string errorMessage)
            => DispatchFromNative(
                () => RaiseLoadingCompleted(errorCode, errorMessage));

        void INativeInFeedAdMobCallbacks.OnAdPaid(AdValue adValue)
            => DispatchFromNative(() => RaiseAdPaid(adValue));

        void INativeInFeedAdMobCallbacks.OnSlotDisplayed(int slotIndex)
            => DispatchFromNative(() => HandleSlotDisplayed(slotIndex));

        void INativeInFeedAdMobCallbacks.OnSlotShowNotReady(int slotIndex)
            => DispatchFromNative(
                () => Debug.LogWarning(
                    $"{nameof(NativeInFeedAdMob)} slot {slotIndex}: "
                    + "Show Called While Not Ready"));

        void INativeInFeedAdMobCallbacks.OnSlotPresentationFailed(
            int slotIndex
          , int errorCode
          , string errorMessage)
            => DispatchFromNative(() => {
        ...
        /// </summary>
        public void Dispose() {
            INativeInFeedAdMobClient releasedClient;
            lock (nativeAdStateLock) {
                if (releasedManaged) return;
