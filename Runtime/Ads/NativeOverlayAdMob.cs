...
namespace RiseOn.NativeAdMob {
    ...
    /// </summary>
    public abstract class NativeOverlayAdMob : NativeAdMob, INativeOverlayAdMobCallbacks {
        ...
        private INativeOverlayAdMobClient client;
        ...
        private protected NativeOverlayAdMob(
          ...
          , float backgroundAlpha)
            : base(adUnitId) {
            var platform = NativeAdMobPlatformRegistry.Installed;
            if (platform == null) {
                Debug.Log(
            ...
            client = platform.CreateOverlay(
                new NativeOverlayAdMobSettings {
                    AdUnitId           = adUnitId
        ...
        public void Release() {
            INativeOverlayAdMobClient       releasedClient;
        ...
        void INativeOverlayAdMobCallbacks.OnLoadingStarted()
            => DispatchFromNative(RaiseLoadingStarted);

        void INativeOverlayAdMobCallbacks.OnLoadingCompleted(int errorCode, string errorMessage)
            => DispatchFromNative(
                () => RaiseLoadingCompleted(errorCode, errorMessage));

        void INativeOverlayAdMobCallbacks.OnAdPaid(AdValue adValue)
        ...
        void INativeOverlayAdMobCallbacks.OnStateChanged(bool isReady, bool isLoading) {
            lock (nativeAdStateLock) {
                if (releasedManaged) return;
        ...
        void INativeOverlayAdMobCallbacks.OnShowNotReady() {}

        void INativeOverlayAdMobCallbacks.OnDisplayed() {
            DispatchFromNative(() => InvokeSafely(OnDisplayed));
        }

        void INativeOverlayAdMobCallbacks.OnPresentationFailed(int errorCode, string errorMessage)
            => DispatchFromNative(() => {
        ...
        void INativeOverlayAdMobCallbacks.OnShowCompleted(
            int showId
          , string errorMessage
          , bool adConsumed) {
