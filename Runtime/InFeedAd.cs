...
namespace RiseOn.NativeAdMob {
    /// <summary>
    /// One InFeedAd is one ad unit id for the life of the app: it owns the shared supply (cache, loads, retry) natively and exposes a fixed array of display slots.<br/>
    /// Index it to get an Item - a stateless handle over one slot - and call Show/Hide/SetPosition there; everything unit-wide (settings, events, disposal) lives here.
    /// </summary>
    public sealed class InFeedAd : BaseAd, IInFeedAdCallbacks {
        ...
        public struct Settings {
            ...
            /// <summary>
            /// The cell's own backdrop, alpha included.<br/>
            /// Transparent is a real answer here: a feed cell that wants no backdrop asks for it.
            /// </summary>
            public Color BackgroundColor;
            /// <summary>
            /// Radius of the cell's rounded corners, in pixels; 0 keeps them square.<br/>
            /// Every asset keeps clear of the curve - inset by just enough that its own corner touches the arc - except the background picture of the scrim layout, which fills the cell and is clipped by the curve instead.<br/>
            /// This is where the unit starts; Initialize carries a radius of its own for a caller that only knows it once the cell is measured.
            /// </summary>
            public int RoundCornerPx;
        }

        /// <summary>
        /// A stateless view over one slot: just (owner, index), so copies are harmless and every bit of state stays inside the owning unit.
        /// </summary>
        ...
        /// <summary>
        /// Sizes every slot of this feed.<br/>
        /// Required before a slot can be positioned or shown - a slot with no size has nowhere to draw - and calling it again resizes them all.
        /// </summary>
        ...
        /// <summary>
        /// The same, with the cell's corner radius.<br/>
        /// The radius travels with the size because a caller measuring a cell on screen only learns both at that moment, and the unit it draws from may have been built long before - warmed at start-up, say.
        /// </summary>
        ...
        /// <summary>
        /// Only for retiring the unit itself (an ad unit id swap); slots need no per-use cleanup.
        /// </summary>
        public void Dispose() {
