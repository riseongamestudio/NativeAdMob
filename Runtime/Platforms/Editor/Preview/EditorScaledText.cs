...
namespace RiseOn.NativeAdMob.Editor {
    /// <summary>
    /// Keeps a label in proportion to the box it lives in.<br/>
    /// <para> The preview renders the same elements at wildly different sizes - a 96dp feed cell and a full screen - so a point size that suits one is wrong in the other: too small in the roomy box, too big in the tight one.<br/>
    /// Best-fit does not solve it either, because it only shrinks text that overflows and leaves a large box looking empty.<br/>
    /// </para> <para> This asks the opposite question: how tall should the text be for this box?<br/>
    /// The answer follows the box on every layout pass.<br/>
    /// </para>
    /// </summary>
    [ExecuteAlways]
