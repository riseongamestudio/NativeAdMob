...
Bản hiện thực iOS của bộ luật chung ở [tài liệu core](../../README.md): Obj-C++
...
- **Thread**: lệnh từ Unity hop về `dispatch_get_main_queue()`
  ...
  C# không marshal (tài liệu core B1). Bảng callback (`RONativeAdListenerCallbacks`)
...
- MAX trên iOS với `InvokeEventsOnUnityMainThread = false` bắn callback trên
  ...
  luật "hide màn che phải ở ngoài player loop" (tài liệu core B4), và cũng
...
- **Half-screen** (cả ad lẫn màn che) chèn `aboveSubview:` cái in-feed cao
  ...
  che đang đứng — khớp hợp đồng "màn che trước, ad sau" của tài liệu core B3.
...
Luật trả giá cho pause (hide phải đến từ thread còn chạy) ở tài liệu core B4 —
...
- Comment đầu `RONativeAdBridge.h` còn nói "C# proxies marshal qua
  `MobileAdsEventExecutor`" — **đã cũ**: core C# không marshal (tài liệu core
